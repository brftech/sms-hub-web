...
class CompaniesService {
  ...
  async getCompanies(options?: {
    ...
    if (options?.search) {
      query = query.or(`public_name.ilike.%${options.search}%,legal_name.ilike.%${options.search}%,billing_email.ilike.%${options.search}%,company_account_number.ilike.%${options.search}%`)
    }
  ...
  async updateCompanySubscription(id: string, subscriptionTier: string, subscriptionStatus: string): Promise<void> {
    const { error } = await this.supabase
      .from('companies')
      .update({ 
  ...
  async getUniqueIndustries(): Promise<string[]> {
    ...
    const industries = data?.map(item => item.industry).filter((industry): industry is string => industry !== null) || []
  ...
  async getUniqueSizes(): Promise<string[]> {
    ...
    const sizes = data?.map(item => item.size).filter((size): size is string => size !== null) || []
  ...
  async getUniqueSubscriptionTiers(): Promise<string[]> {
    ...
    const tiers = data?.map(item => item.subscription_tier).filter((tier): tier is string => tier !== null) || []
