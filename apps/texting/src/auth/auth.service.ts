...
export class AuthService {
  ...
  async createTempSignup(signupData: SignupData) {
    ...
    if (error) {
      throw new BadRequestException(`Failed to create signup: ${error.message}`)
    }
  ...
  async verifyCode(verificationData: VerificationData) {
    const { temp_signup_id, verification_code } = verificationData

    // Get temp signup
    const { data: tempSignup, error: tempError } = await this.supabaseService.client
      .from('temp_signups')
      .select('*')
  ...
  async createUserFromTempSignup(tempSignupId: string) {
    ...
    const accountNumber = await this.supabaseService.generateAccountNumber(hub.name)
