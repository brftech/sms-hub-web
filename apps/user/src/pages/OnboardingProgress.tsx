...
import { useCurrentUserCompany, useBrands, useCurrentUserCampaigns, useUserProfile } from '@sms-hub/supabase/react'
import { useCustomerByCompany } from '@sms-hub/supabase/customer-queries'
...
const StepCard = styled.div<{ $completed: boolean; $current: boolean }>`
  background: ${props => 
    props.$completed ? 'rgba(71, 85, 105, 0.25)' : 
    props.$current ? 'rgba(71, 85, 105, 0.2)' : 
    'rgba(15, 23, 42, 0.15)'};
  backdrop-filter: blur(10px);
  border: 1px solid ${props => 
    props.$completed ? 'rgba(148, 163, 184, 0.3)' : 
    props.$current ? 'rgba(148, 163, 184, 0.25)' : 
    'rgba(71, 85, 105, 0.2)'};
  border-radius: 12px;
  padding: 1.25rem;
  ...
  &:hover {
    ...
    border-color: ${props => 
      props.$completed ? 'rgba(148, 163, 184, 0.4)' : 
...
const StepIcon = styled.div<{ $completed: boolean; $current: boolean }>`
  ...
  border-radius: 12px;
  background: ${props => 
    props.$completed ? 'linear-gradient(135deg, #475569, #64748b)' : 
    props.$current ? 'linear-gradient(135deg, #64748b, #94a3b8)' : 
    'rgba(71, 85, 105, 0.3)'};
  display: flex;
  align-items: center;
  justify-content: center;
  color: ${props => props.$completed || props.$current ? '#f8fafc' : '#cbd5e1'};
  box-shadow: ${props => 
...
export function OnboardingProgress() {
  ...
  const steps = [
    ...
    },
    {
      ...
      route: '/onboarding',
      action: customer?.stripe_customer_id && !customer?.stripe_subscription_id ? 'Complete Payment' : 'Setup Payment'
    },
    {
      id: 'personal',
    ...
    },
    {
      ...
      description: 'Register your brand with The Campaign Registry for better deliverability.',
      icon: <Building />,
      completed: brands?.some(b => b.status === 'approved'),
    ...
    },
    {
      ...
      icon: <Megaphone />,
      completed: campaigns?.some(c => c.status === 'approved'),
      ...
      <PageContainer>
        <CompletionCelebration>
          ...
          <Subtitle style={{ fontSize: '1.125rem', marginBottom: '2rem' }}>
            You've completed your {hubConfig.displayName} onboarding! Your SMS platform is ready to use.
          </Subtitle>
        ...
        {steps.map((step, index) => {
          ...
          return (
            <StepCard
              ...
              style={{ cursor: (step.completed || isCurrent) ? 'pointer' : 'default' }}
            >
              <StepHeader>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ 
                    ...
                    {step.completed && (
                      <>
                        <span>•</span>
                        <span style={{ color: '#94a3b8', fontWeight: '600' }}>Completed</span>
                      </>
                    )}
                    {isCurrent && (
                      <>
                        <span>•</span>
                        <span style={{ color: '#cbd5e1', fontWeight: '600' }}>Current</span>
                      </>
                    )}
                  </div>
                </div>
