...
export const useCompanySubscription = (companyId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!company?.customer_id) throw new Error('No customer associated with this company')
...
export const useUpcomingInvoice = (companyId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await getSupabaseClient().functions.invoke('get-upcoming-invoice', {
...
export const useUpdateSubscription = () => {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const supabase = getSupabaseClient()
      const { data, error } = await getSupabaseClient().functions.invoke('update-subscription', {
        body: params
      })
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['subscription', variables.companyId] })
      queryClient.invalidateQueries({ queryKey: ['upcoming-invoice', variables.companyId] })
    },
  })
}
...
export const useCancelSubscription = () => {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const supabase = getSupabaseClient()
      const { data, error } = await getSupabaseClient().functions.invoke('cancel-subscription', {
        body: params
      })
...
export const useResumeSubscription = () => {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const supabase = getSupabaseClient()
      const { data, error } = await getSupabaseClient().functions.invoke('resume-subscription', {
        body: params
      })
...
export const useUpdatePaymentMethod = () => {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const supabase = getSupabaseClient()
      const { data, error } = await getSupabaseClient().functions.invoke('update-payment-method', {
        body: params
      })
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['payment-methods', variables.companyId] })
    },
  })
}
...
export const usePaymentMethods = (companyId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await getSupabaseClient().functions.invoke('get-payment-methods', {
        body: { customerId: customer.stripe_customer_id }
      })
...
export const useInvoices = (companyId: string, limit = 10) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await getSupabaseClient().functions.invoke('get-invoices', {
        body: { 
...
export const useCreatePortalSession = () => {
  return useMutation({
    mutationFn: async (params: {
      ...
      const supabase = getSupabaseClient()
      const { data, error } = await getSupabaseClient().functions.invoke('create-portal-session', {
        body: params
      })
...
function getSubscriptionLimits(tier: string) {
  const limits: Record<string, { sms_monthly: number; users: number; phone_numbers: number }> = {
