...
export const FormBuilder: React.FC<FormBuilderProps> = ({
   ...
   */
  const validateField = useCallback(async (field: FormFieldType, value: unknown): Promise<string | null> => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submissionData = includeTurnstile 
  ...
  const renderFields = () => {
    if (layout === 'two-column') {
      const leftColumn = visibleFields.filter((_, i) => i % 2 === 0);
      const rightColumn = visibleFields.filter((_, i) => i % 2 === 1);
