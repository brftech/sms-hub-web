...
export const FormField: React.FC<FormFieldProps> = ({
  ...
  const ariaDescribedBy =
    [hasError ? errorId : null, field.helpText ? helpId : null].filter(Boolean).join(" ") ||
    undefined;
          ...
          <div className="space-y-2">
            {field.options?.map((option) => (
              <div key={option.value} className="flex items-center">
                ...
                />
                <label htmlFor={`${fieldId}-${option.value}`} className="ml-2 text-white">
                  {option.label}
                </label>
              </div>
            ))}
  ...
  return (
    <div className={`mb-6 ${className}`}>
      {/* Label with inline error */}
      {field.type !== "radio" && (
        <div className="flex items-center justify-between mb-2 min-h-[24px]">
          <Label htmlFor={fieldId} className="text-white">
            {field.label}
            {isRequired && <span className="text-red-500 ml-1">*</span>}
            {field.optional && <span className="text-zinc-500 text-sm ml-2">(optional)</span>}
          </Label>
          {/* Error Message - Always rendered for consistent spacing */}
          <span
            id={errorId}
            className={`text-sm ${hasError ? "text-red-500" : "text-transparent"}`}
