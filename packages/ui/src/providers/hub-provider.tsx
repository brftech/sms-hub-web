...
import { detectHubFromDomain, detectHubFromPath, detectHubFromPort } from '@sms-hub/utils'
...
export const HubProvider: React.FC<HubProviderProps> = ({
  ...
  const [currentHub, setCurrentHub] = useState<HubType>(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const hubFromPort = detectHubFromPort(parseInt(window.location.port) || 3000)
      const hubFromQuery = new URLSearchParams(window.location.search).get('hub') as HubType
      
      return hubFromQuery || hubFromPath || hubFromDomain || hubFromPort || defaultHub || 'percytech'
    }
    
    return defaultHub || 'percytech'
  })
