-- Migration: 20250831165747_disable_all_rls_policies.sql
-- Purpose: Disable ALL Row Level Security policies on all tables
-- Description: Remove RLS from all public schema tables while preserving native Supabase auth

-- 1. Disable RLS on all tables in public schema
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            RAISE NOTICE 'Table % still has RLS enabled', table_record.tablename;
        END IF;
    END LOOP;
    
    IF rls_enabled_count = 0 THEN
        RAISE NOTICE 'SUCCESS: All RLS policies have been disabled and removed!';
        RAISE NOTICE 'All tables in public schema now have RLS disabled.';
        RAISE NOTICE 'Native Supabase auth system remains intact.';
    ELSE
...
COMMENT ON SCHEMA public IS 'RLS has been completely disabled on all tables. Access control should be handled at the application level.';
