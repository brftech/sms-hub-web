...
COMMENT ON TABLE user_profiles IS 'User profiles linked to auth.users';
COMMENT ON TABLE verifications IS 'Verification codes for SMS/email authentication';
...
-- Note: RLS policies are intentionally not included as they are currently disabled
-- When ready to enable RLS, policies should be added in a separate migration